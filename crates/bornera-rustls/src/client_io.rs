//! Caller-driven encrypted and plaintext progression for client sessions.
...
use crate::{
    RustlsClientSession, RustlsPeerClosure,
...
impl RustlsClientSession {
    ...
    /// Irreversibly accepts bounded request plaintext and returns its exact count.
    pub fn write_plaintext(&mut self, plaintext: &[u8]) -> Result<usize, TransportError> {
        ...
        if let Err(error) = self.refresh(TransportFailurePhase::Write) {
            self.failure.get_or_insert(error.diagnostic());
        }
        Ok(written)
    }

    /// Copies pending TLS records into a caller buffer and returns the produced count.
